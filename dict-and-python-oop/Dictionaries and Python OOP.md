...
> 这篇文章**不**适合下面的读者：
>
> - 仅希望知道 Python 中 OOP 相关的语法以通过考试
> - 希望以这篇文章作为语法参考
> - 希望了解 Python 之外**其他语言**中 OOP 的实现
>
> 这篇文章的语言难度**略高于**课程讲义。
>
> 你需要在阅读过程中常常自己尝试写一些代码，尤其是文章中希望你这么做时。

## Introduction

This article offers a look under the hood of OOP in Python. I will talk *a lot* about language-specific features that only a language as idiosyncratic as Python has. I will try to untangle the general idea of OOP from the implementation details of the Python language. However, the nature of the Python language makes the path inevitably winding, and we *will* be looking at implementation details that many tutorials skip. Keep in mind that many of these ideas may not apply directly to other languages.
...
It's important to note that this article is not a handbook of concepts and syntax, but an attempt to take a journey of struggles and dilemmas before we come to what a typical tutorial arrives at. It is up to you to synthesize and internalize what you've learned in this journey. Keep curious, and the journey will be rewarding.
...
​	However, this is not good enough because `coke_vm[0]` is pure nonsense: what on earth does it mean to "take the first element" or "use index 0 on" a vending machine? The code itself lacks meaning. Or, it is not **self-explanatory**.

​	The attempt above falls short because we are using integer indexing, and the integers themselves do not mean anything. We want to find a readable and self-explanatory way to organize these functions.

​	Well, if integer indexing doesn't work, you might think, why not use more readable keys, like strings? `coke_vm['restock']` definitely looks better than `coke_vm[0]`. This is where dictionaries come into play.
...
>
> Also, you can think of the dot operator as an apostrophe 's'. For example, `vm.deposit` means "`vm`'s `deposit`". In Chinese, a more direct analogy is "的".
...
​	By doing so, Python organizes these functions under **the class's namespace**, i.e. these functions belong to the `Tree` class. To access them, we also use the dot operation: `Tree.is_leaf(tr)`.
...
> [!Tip]
>
> Objects created by calling a class's name as a function are called **instances** of the class. Most of the time, *objects* and *instances* are used interchangeably in this article.
...
> [!Tip]
>
> You may wonder how an instance knows where to find the function to be invoked. For instance:
>
> ```python
> class Foo():
>     def func(obj):
...
> 
> obj = Foo()
> obj.func() # how does OBJ know which function to call?
> ```
>
> The output will be `Foo!`. This is because during object creation, Python tells the object which class it belongs to. In this example, `obj` gets informed by Python to remember it belongs to `Foo`, and look for functions in that namespace.
>
> We'll talk about this in more detail later in this article.
...
>
> But most of the time, we don't pay attention to this distinction. When someone says *constructor*, they probably mean the `__init__` method unless they're talking about advanced Python features.

​	As is often the case, initializers take arguments. On such occasions, we simply call the class (remember that classes can be called like functions!) with the desired arguments as if you were calling the initializer.
...
​	Again, I recommend that you pause here to reflect on how classes save us from the dilemma. It may be beneficial to still think in terms of dispatch dictionaries and floating functions and then map concepts to the `class` syntax when writing code related to classes. It may also help to decode the `class` syntax to dictionaries and floating functions when reading code using classes.

​	Before proceeding to the next section, do try writing some code with classes. It is something that takes some time to get familiar with even though you're fully comfortable with the ideas behind it (which I hope you are!).
